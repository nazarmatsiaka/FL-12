...
$('#header').next('div').css({"border-style": "double", "padding": "20px"});
...
// in #list-1
  ...
  $('#list-1').children(':hidden:not(.cloned)').show();

  // hide empty li
  $('#list-1').children(':empty').remove();

// in #list-3
  // for all even li set margin-left -20px
  $('#list-3').children(':odd').css({"margin-left": "-20px"});

  // for the first li set any different color
  $('#list-3').children(':first').css({"color": "blue"});

  // for all li with index > 5 set color to #ccc
  $('#list-3').children(':gt(5)').css({"color": "#ccc"});

// show ul which is parent
$('*').parent('ul').css({ "border": "1px solid green" });
...
$('p').children('b:only-child').css({"font-size": "36px"});
...
$('img').parent('div').first().css({"float": "left", "border": "1px solid red"});
